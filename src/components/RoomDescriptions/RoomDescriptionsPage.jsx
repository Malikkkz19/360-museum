...
const roomsData = [
  ...
  },
  {
    id: "room5",
    name: "ЗАЛ 5",
    description: "Зал славы посвящен выдающимся людям, в разное время имевшим прямое отношение к Академии. Генерал-полковник Кизим Л.Д., генерал-лейтенант Шумилин А.А., генерал-лейтенант Медведев Д.А., генерал-майор Гудилин В.Е., полковник Петухов Г.Б., генерал армии Антонов А.И., космонавт Шаргин Ю.Г., генерал-полковник Поповкин В.А. нашли достойное отражение на стендах и витринах зала, где представлены различные материалы, рассказывающие об этих людях, личные вещи, документы, награды.",
    images: [
      "/images/5.png", 
      "/images/canada.jpg", 
      "/images/vka.jpg",
  ...
  },
  {
    id: "room6",
    name: "ЗАЛ 6",
    description: "Лекционный, учебно-методический зал служит для проведения лекций, семинаров, концференций и других. мероприятий, связанных с историей Академии и развития космической техники, и имеет профориентационную направленность. В зале представлены материалы по структуре, составу Академии в настоящее время, назначению и задачам входящих в неё факультетов.",
    images: [
      "/images/6.png",
      "/images/Image1.jpg", 
      "/images/vka.jpg",
...
function RoomDescriptionsPage() {
  ...
  // Добавляем эффект для предотвращения прокрутки страницы при открытом модальном окне
  useEffect(() => {
    if (modalVisible) {
  ...
  return (
    <div className="room-descriptions-page">
      ...
      <div className="room-descriptions-content">
        ...
        {/* Галерея карточек залов - симметрично 3x2 */}
                ...
                >
                  {room.images.map((image, index) => (
                    <div key={index} className="room-card-slide">
                      <img src={image} alt={`${room.name} - фото ${index + 1}`} />
                    </div>
                  ))}
                </Carousel>
              </div>
        ...
        {/* Модальное окно с подробным описанием зала */}
                ...
                <div className="room-modal-carousel">
                  ...
                  >
                    {activeRoomData.images.map((image, index) => (
                      <div key={index}>
                        <img src={image} alt={`${activeRoomData.name} - фото ${index + 1}`} />
                      </div>
                    ))}
                  </Carousel>
                </div>
